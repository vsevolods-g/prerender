...
class Chrome {
    ...
    async connectToTab({ targetUrl }) {
        try {
            if (!this.browser) {
                const browser = await CDP({ target: this.webSocketDebuggerURL, port: this.debuggingPort });
                this.browser = browser;
            }

            const { browserContextId } = await this.browser.Target.createBrowserContext()
            const { browserContextIds } = await this.browser.Target.getBrowserContexts();
            const { targetId } = await this.browser.Target.createTarget({
                url: 'about:blank',
                browserContextId: browserContextIds[0] || browserContextId,
            });
            const { Page, Network } = await CDP({ target: targetId, port: this.debuggingPort });

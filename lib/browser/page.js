...
class Page {
    ...
    async connectToTab() {
        this.pageInstance = await CDP({ target: this.frameId, port: this.debuggingPort });
    }
    ...
    async waitForPageToFullRender() {
        ...
        await this.waitForNextTimeCheck(process.env['TIMEOUT_FOR_PAGE_FULLY_LOAD']);
    }
    ...
    */
    ...
    gatherPendingNetworkRequests({ Network }) {
        // TODO: Block unnecessary requests, currently breaks browser.
        ...
        //         Network.continueInterceptedRequest(interceptOptions);
        //     } catch(e) {
        //         console.log(e)
        //     }
        // });
        ...
        Network.requestWillBeSent((request) => {
            const { type, requestId, request: { url } } = request;
            if (type === 'Fetch' && url.includes('graphql') || type === 'Image') {
    ...
    async waitForNoPendingRequests() {
        ...
        while (this.pendingRequests.length > 0 && checkTries < 10) {
            if (JSON.stringify(this.pendingRequests) === JSON.stringify(prevPendingRequests)) {
                checkTries++;
            } else {
                checkTries = 0;
            ...
            await new Promise((resolve) => setTimeout(resolve, process.env['TIMEOUT_FOR_PENDING_REQUEST_CHECK']));
        }
    }
    ...
    async takeScreenshot({ Page, targetUrl }) {
        ...
        fs.writeFileSync(`./var/screenshots/${pathname.replace(/\//g, '-')}.png`, Buffer.from(data, 'base64'));
    }
}

...
class MongoDBClient {
    ...
    async getCachedPages(pageSize, page, urlFilter, sortBy, sortDirection) {
        const skip = (page - 1) * pageSize;
        const query = urlFilter ? { "key": { $regex: `.*${urlFilter}.*` } } : {};
        const formattedSortDirection = sortDirection === 'DESC' ? -1 : 1;
        const sort = sortBy ? { [sortBy]: formattedSortDirection } : { _id: -1 };

        return await this.collection
            .find(query)
            .project({ status: 1, expirationTime: 1, key: 1, createdAt: 1 })
            .sort(sort)
            .skip(skip)

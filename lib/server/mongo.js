...
class MongoDBClient {
    constructor() {
        ...
        this.expirationTimeByStatusFromDB = {
            200: 'statusCode200',
            404: 'statusCode404',
            301: 'statusCode301',
            302: 'statusCode302'
    ...
    async connect() {
        ...
        this.domainSettingsCollection = this.db.collection('domains');
    }
    ...
    async updateRecacheStatus(status, key) {
        try {
            await this.collection.updateOne(
                { key },
                {
                    $set: {
                        recacheStatus: status,
                    },
                }
            );
        } catch (error) {
            console.log('Error while updating recache status.');
            console.log('url: ', url);
    ...
    async insertCache(params) {
        ...
        const existingCache = await this.getCache(url);
        ...
        const domainSettings = await this.domainSettingsCollection.findOne({domain: domainName})
        
        const cacheTimeInDays = domainSettings?.values[0][this.expirationTimeByStatusFromDB[status]] || this.expirationTimeByStatus[status];
        ...
        const ttl = new Date(
            Date.now() + cacheTimeInDays * 24 * 60 * 60 * 1000
        );
    ...
    async getDomainsForOrganizations(organizationIds) {
        try {
            ...
            const organizationsCollection =
                mongo.db.collection('organizations');
    ...
    async getUniqueDomains() {
        ...
        return domains.map((d) => d.domain.replace('www.', '')).filter(Boolean);
    }
}
